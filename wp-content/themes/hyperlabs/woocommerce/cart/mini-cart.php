...
<?php if ( ! WC()->cart->is_empty() ) : ?>
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<li class="hl_minicard_itm woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					$product = $cart_item['data'];
						...
						echo '<div class="mini-cart-product-price">Цена: </span>' . wc_price($product->get_price() * $cart_item['quantity']) . ' </span></div>';


						 // Display product quantity with increase and decrease buttons
						 echo '<div class="mini-cart-quantity">';
						 echo '<span class="quantity-label">Quantity:</span>';
						 echo '<div class="hl__quantity d-flex"><button class="quantity-decrease" data-cart-key="' . $cart_item_key . '">-</button>';
						 echo '<span class="quantity">' . $cart_item['quantity'] . '</span>';
						 echo '<button class="quantity-increase" data-cart-key="' . $cart_item_key . '">+</button></div>';
	...
	<?php 
	echo '<a class="continue-shopping-button" href="' . esc_url(home_url('/shop')) . '">Continue Shopping</a>';
	 // Add "Verify" button
	 echo '<a class="verify-button" href="' . esc_url(wc_get_checkout_url()) . '">Verify</a>';?>
	 </div>
